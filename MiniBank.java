  ...
  public static void registration() throws IOException
    {
      ...
      while(menu)
      {
        ...
        boolean Number = true;
        while(Number)
        { // loop is necessary to exclude chance of account number coincidences
          registrationData[5] = ""; /* subsitute null with "" */
          for(int i = 0; i < 5; i++)
          { // generating account number that consists of 5 numbers
  ...
  public static void login() throws IOException
    {
      try
      {
        ...
        thisAccount = decryption(readFile(Base64.getEncoder().encodeToString(allAccountNumbers[flag].getBytes()) + ".txt"));
        /*
              ...
              try
              {
                if(allAccountNumbers[i].compareTo(accountNumber) == 0)
                {
                  ...
                  try
                  {
                    ...
                    if(Double.parseDouble(thisAccount[6]) - amount >= 0)
                    { // check if it is enough funds
                      thisAccount[6] = String.valueOf(Double.parseDouble(thisAccount[6]) - amount); // subtract amount
                      
                      accountToTransfer[6] = String.valueOf(Double.parseDouble(accountToTransfer[6]) + amount); // add to another account

                      receipts(String.valueOf(amount), "Transfer", accountToTransfer[5]);
                      writeFile(encryption(thisAccount), encryption(thisAccount)[5] + ".txt", ""); // save and encrypt

                      writeFile(encryption(accountToTransfer), encryption(accountToTransfer)[5] + ".txt", ""); // save and encrypt 
          ...
          public static void viewAll(boolean direction)
            {
              ...
              if(direction)
              { // if true display from first element to last
                for(int i = 0; i < allReceipts.length; i++)
                {
              ...
              { // if false backwards. This block simplifies sorting data. You don't need to
          ...
          public static void sorting(int criteria)
            {
              ...
              if(criteria == 1)
              { // display from earliest to latest
                viewAll(true);
              }
              else
              { // display from biggest to smallest
                viewAll(false);
              }
            } //end sorting methid
          ...
          public static void writeFile(String[] data, String name, String extraData) throws IOException
            {
              ...
              if(extraData != "")
              { // added to write a new email to email data base
